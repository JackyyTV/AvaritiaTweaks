...
public class TileInfinitato extends TileTinyPotato {
    ...
    public void interact(EntityPlayer player, EnumHand hand, ItemStack stack, EnumFacing side) {
        ...
        if(name.equalsIgnoreCase("shia labeouf") && !worldObj.isRemote && nextDoIt == 0) {
            nextDoIt = 40;
            worldObj.playSound(player, player.getPosition(), ModUtils.getSoundFromName("botania:doit"),
                    SoundCategory.BLOCKS, 2.5F, 0.7F);
        }
        double radius = 10.5;
        int time = 3600;
        List<EntityLivingBase> inspired = worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
    ...
    public void update() {
        if(jumpTicks > 0) {
            ...
            if (jumpTicks == 20 || jumpTicks == 0) {
                this.worldObj.createExplosion(null, getPos().getX() + 0.5, getPos().getY(), getPos().getZ() + 0.5, 0.0f, true);
            }
        }
        if(nextDoIt > 0)
            nextDoIt--;
